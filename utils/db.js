...
class DB {
    constructor() {
        ...
        this.adminMethod = null; // 'client' or 'sudo'
        this.pool = null;
        this.query = null;
    ...
    async executeAsPostgres(sqlCommand) {
        try {
    ...
    async sudo_checkIfUserExists(username) {
    ...
    async sudo_checkIfDatabaseExists(dbname) {
    ...
    async sudo_createUser(username, password) {
        try {
            ...
            const createUserResult = await this.executeAsPostgres(`CREATE USER "${username}" WITH PASSWORD '${password}' CREATEDB;`);
    ...
    async sudo_createDatabase(dbName, owner) {
        try {
            ...
            const createDbResult = await this.executeAsPostgres(`CREATE DATABASE "${dbName}" OWNER "${owner}";`);
    ...
    async createDatabase(dbName, owner) {
        if (this.adminMethod === 'client') {
